...
Current state: ~10,800 lines of Python across 25 modules. 394 tests passing.
One backend (C).
...
Checks: E390 (explain row count mismatch), E391 (duplicate explain row),
E392 (explain reference not found), E393 (believe without ensures),
E394 (explain operation not recognized), E366 (recursive without terminates),
W321 (explain text missing references), W322 (duplicate near-miss),
W323 (ensures without explain), W324 (ensures without requires),
W325 (explain without ensures), W326 (unbounded recursion).
...
| Risk | Mitigation |
|------|------------|
| Bootstrap compiler bugs | Extensive test suite (394+ tests), fix bugs as found during self-hosting |

...
**v0.5 — Turbo Runtime** (see `0.5-PLAN.md`):
...
**v0.7 — IO and Parse** (see `0.7-PLAN.md`):
...
| ASM backend | `archive/asm/` | Archived in v0.4.1; C backend is sufficient |
...
| Pure verbs | `reads`, `creates`, `saves` — standalone verbs in the pure family | Working |
...
| Risk | Mitigation |
|------|------------|
| Bootstrap compiler bugs | Extensive test suite (462+ tests), fix bugs as found during self-hosting |
...
| Cross-architecture | Turbo runtime is portable C with small `#ifdef` for CRC32; no architecture-specific ASM |

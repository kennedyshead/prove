...
Current state: ~12,400 lines of Python across 25 modules. 590 tests passing.
One backend (C).
...
## Part 1: Stdlib and Runtime

Three releases building up to the full stdlib:

**v0.5 — Turbo Runtime and AST Optimizer** (see `0.5-PLAN.md`):

| Component | Functions | Purpose |
|-----------|-----------|---------|
| Arena allocator | 4 | Bump-pointer allocator (eliminates malloc + RC) |
| Fast hash | 1 | Hardware CRC32 hash (x86_64 + ARM64 + fallback) |
| String intern | 3 | String deduplication (pointer-equality comparisons) |
| AST optimizer | — | TCO, iterator fusion, copy elision, match compilation, inlining, dead branch elimination |

**v0.6 — Core Stdlib** (see `0.6-PLAN.md`):

| Module | Functions | Purpose |
|--------|-----------|---------|
| `Character` | 7 | Character classification |
| `Text` | 18 | String ops + Builder |
| `Table` | 8 | Hash map (uses turbo hash) |

**v0.7 — IO, Parse, and C FFI** (see `0.7-PLAN.md`):

| Component | Functions | Purpose |
|-----------|-----------|---------|
| `InputOutput` (ext) | +14 | Channels: console, file, dir, system, process |
| `Parse` | 4 | Format codecs (TOML, JSON) |
| C FFI | — | Foreign function declarations for calling C libraries |

**Prerequisite:** v0.3 (legacy cleanup), v0.4 (language features), and
v0.5–v0.7 (stdlib) must be complete before compiler modules can be
written in Prove.

**Status:** v0.5–v0.8.3 are complete. The stdlib (arena, hash, intern,
Character, Text, Table, IO, Parse) is ready. The formatter has type
inference (v0.8), the lint system has been overhauled (v0.8.1), `proof`
has been migrated to `explain` (v0.8.2), and remaining lint diagnostics
are implemented (v0.8.3). 590 tests passing.
...
Source → Tokens → Errors → Ast → Lexer → Types → Symbols → Parser
    → Checker → Prover → Optimizer → CTypes → CEmitter → CCompiler
    → CRuntime → Testing → Config → Builder → Cli
```
...
**Python:** `tokens.py` (206 lines) — `TokenKind` enum with 67 variants,
...
type TokenKind is
  ...
  | Transforms | Inputs | Outputs | Validates | Matches_ | Types_
  | Ensures | Requires | Explain | Terminates_ | Trusted_
...
**Est.:** ~130 lines of Prove (algebraic type is verbose but clear)

### 2.3 `Errors` — Diagnostics

**Python:** `errors.py` (230 lines)
...
**Python:** `ast_nodes.py` (474 lines) — 40+ frozen dataclasses.
...
**Est.:** ~270 lines of Prove (mostly type definitions)

### 2.5 `ResolvedTypes` — Type System

**Python:** `types_.py` (270 lines)
...
**Est.:** ~170 lines of Prove

### 2.6 `Symbols` — Symbol Table

**Python:** `symbols.py` (201 lines)
...
**Est.:** ~500 lines of Prove (character classification via `Character`, string building via `Text`)

### 2.8 `Parser` — Parsing

**Python:** `parser.py` (1,743 lines) — Pratt expression parser + recursive
...
- `parse_explain` — `explain` block with rows

**Est.:** ~1,300 lines of Prove

### 2.9 `Checker` — Semantic Analysis

**Python:** `checker.py` (1,756 lines) — two-pass analyzer.
...
**Est.:** ~1,250 lines of Prove

### 2.10 `Prover` — Explain and Contract Verification

**Python:** `prover.py` (332 lines)

| Function | Signature |
|----------|-----------|
| `verify_explain` | `transforms verify_explain(fd FunctionDef) List<Diagnostic>` |

Checks: E391 (duplicate explain entry), E392 (explain entries < ensures count),
E393 (believe without ensures), E394 (explain condition must be Boolean),
E366 (recursive without terminates),
W321 (explain text missing references), W322 (duplicate near-miss),
W323 (ensures without explain — added v0.8.3), W324 (ensures without requires),
W325 (explain without ensures), W326 (recursion depth may be unbounded — added v0.8.3).

**Est.:** ~240 lines of Prove

### 2.11 `Optimizer` — AST Optimizer

**Python:** `optimizer.py` (581 lines)

| Function | Signature |
|----------|-----------|
| `optimize` | `transforms optimize(module_ Module_, st SymbolTable) Module_` |

Performs source-level optimizations on the checked AST:
- Tail call optimization (TCO) — rewrite tail-recursive calls as loops
- Iterator fusion — merge chained `map`/`filter` into single pass
- Copy elision — avoid copies when pure function results are immediately bound
- Match compilation — convert nested matches into decision trees
- Small function inlining — inline single-expression pure functions
- Dead branch elimination — remove statically unreachable arms

The optimizer returns a modified AST that the C Emitter consumes.

**Est.:** ~420 lines of Prove

### 2.12 `CTypes` — C Type Mapping

**Python:** `c_types.py` (189 lines)
...
### 2.13 `CEmitter` — C Code Generation

**Python:** `c_emitter.py` (1,814 lines) — the largest module.
...
arm. String building uses `Builder` from `Text` module.
...
**Est.:** ~1,350 lines of Prove

### 2.14 `CCompiler` — Invoking gcc
...
### 2.15 `CRuntime` — Runtime File Copying

**Python:** `c_runtime.py` (59 lines)
...
**Est.:** ~45 lines of Prove

### 2.16 `Testing` — Contract Test Generation
...
### 2.17 `Config` — Project Configuration

**Python:** `config.py` (96 lines)
...
### 2.18 `StdlibLoader` — Stdlib Loading

**Python:** `stdlib_loader.py` (400 lines)
...
**Est.:** ~280 lines of Prove

### 2.19 `Builder` — Build Pipeline

**Python:** `builder.py` (165 lines)
...
Pipeline: find sources → lex → parse → check → prove → optimize → emit C →
copy runtime → compile C → binary.

**Est.:** ~120 lines of Prove

### 2.20 `Cli` — Command Line Interface

**Python:** `cli.py` (607 lines) — uses Click framework.
...
`prove test [path]`. (`prove new` is deferred to v1.3.)

**Est.:** ~310 lines of Prove
...
| Source | 51 | 30 | — |
| Tokens | 206 | 130 | Map |
| Errors | 230 | 150 | Text, InputOutput |
| Ast | 474 | 270 | — |
| Lexer | 656 | 500 | Character, Text |
| ResolvedTypes | 270 | 170 | Map |
| Symbols | 201 | 140 | Map |
| Parser | 1,743 | 1,300 | — |
| Checker | 1,756 | 1,250 | Table |
| Prover | 332 | 240 | Table |
| Optimizer | 581 | 420 | — |
| CTypes | 189 | 140 | Text |
| CEmitter | 1,814 | 1,350 | Text |
| CCompiler | 84 | 60 | InputOutput, Text |
| CRuntime | 59 | 45 | InputOutput |
| Testing | 674 | 500 | Text |
| Config | 96 | 80 | Parse, InputOutput |
| StdlibLoader | 400 | 280 | InputOutput, Text |
| Builder | 165 | 120 | InputOutput, Text |
| Cli | 607 | 310 | InputOutput |
| **Total** | **~10,600** | **~7,500** | |

The Prove version is estimated at ~70% of the Python line count. Algebraic
...
### Phase 1: Runtime and Stdlib (C-backed)

Completed in v0.5 (turbo runtime), v0.6 (core stdlib), v0.7 (IO + Parse).
...
| `Source` | — |
| `Tokens` | Source, Map |
| `Errors` | Source, Text, InputOutput |
| `Ast` | Source, Tokens |
| `ResolvedTypes` | Map |
...
| Module | Depends on |
|--------|------------|
| `Lexer` | Source, Tokens, Errors, Character, Text |
...
| `Optimizer` | Ast, ResolvedTypes, Symbols |
...
| Module | Depends on |
|--------|------------|
| `CTypes` | ResolvedTypes, Text |
| `CEmitter` | Ast, ResolvedTypes, Symbols, CTypes, Text |
| `CCompiler` | InputOutput, Text |
| `CRuntime` | InputOutput |
| `Testing` | Ast, ResolvedTypes, Symbols, CTypes, Text |
...
| `StdlibLoader` | InputOutput, Text, Ast, Symbols |
| `Builder` | all above |
...
| Feature | Python module | Deferred to | Reason to defer |
|---------|--------------|-------------|-----------------|
| Formatter | `formatter.py` | v1.1 | Nice-to-have, not needed for compilation |
| AST view | `cli.py` (view cmd) | v1.1 | Debugging tool, not critical path |
| LSP server | `lsp.py` | v1.2 | Needs `pygls`, not critical path |
| Project scaffolding | `project.py` | v1.3 | Simple, low priority |
| Highlights export | (new in v1.3) | v1.3 | Machine-readable keyword data |
| Lexer export tool | `export.py` (v0.9) | stays in Python | Generates companion project files, no need to self-host |
| ASM backend | `archive/asm/` | — | Archived in v0.4.1 |
...
| `Module.function()` calls | Namespaced stdlib access, requires explicit import — unimported functions are errors | Working |
| Pure verbs | `reads`, `creates`, `matches` — standalone verbs in the pure family | Working |
| `type X is binary` | C-backed types with no Prove-visible fields | Working |
| `comptime` blocks | Embedded constants | Working |
| `explain` blocks | Controlled natural language documentation of `from` steps | Optional |
| `terminates` | Required measure expression for recursive functions | Working |
| `trusted` | Explicit opt-out for verification chain | Working |
...
| Risk | Mitigation |
|------|------------|
| Bootstrap compiler bugs | Extensive test suite (590 tests), fix bugs as found during self-hosting |
| Missing stdlib function | Add to C runtime as needed; stdlib is extensible |
| Performance regression | Turbo runtime (arena, intern, CRC32 hash) for hot paths; C backend + gcc -O2 for everything else |
| Circular module deps | Dependency graph is acyclic by design |
| Large compilation units | Each module compiles independently |
| String-heavy code gen | Text.Builder prevents O(n^2) concat |
| Cross-architecture | Turbo runtime is portable C with small `#ifdef` for CRC32 |

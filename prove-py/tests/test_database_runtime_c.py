"""Tests for the Database C runtime module."""
    ...
    def test_create_and_validate(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            ...
            int main(void) {
                ...
                Prove_Result r = prove_db_create(path);
                printf("create_ok=%%d\\n", prove_result_is_ok(r) ? 1 : 0);
                printf("validates=%%d\\n", prove_db_validates(path) ? 1 : 0);
                /* Non-existent path */
                Prove_String *bad = prove_string_from_cstr("%s/no/such/deep/path");
                printf("bad_validates=%%d\\n", prove_db_validates(bad) ? 1 : 0);
                return 0;
            }
        """ % (str(tmp_path), str(tmp_path)))
        result = compile_and_run(runtime_dir, tmp_path, code, name="db_create")
    ...
    def test_table_constructor(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            ...
            int main(void) {
                ...
                Prove_DatabaseTable *t = prove_db_table_new(name, 2, cols);
                printf("name=%s\\n", t->name->data);
                ...
                prove_db_table_add_variant(t, prove_string_from_cstr("Ok"), vals);
    ...
    def test_save_load_roundtrip(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            ...
            int main(void) {
                ...
                Prove_Result cr = prove_db_create(path);
                if (!prove_result_is_ok(cr)) { printf("FAIL create\\n"); return 1; }
                Prove_Database *db = (Prove_Database *)prove_result_unwrap_ptr(cr);

                /* Create table */
                Prove_String *cols[2];
                cols[0] = prove_string_from_cstr("code");
                cols[1] = prove_string_from_cstr("message");
                Prove_DatabaseTable *t = prove_db_table_new(
                    prove_string_from_cstr("status"), 2, cols);

                Prove_String *v1[2];
                v1[0] = prove_string_from_cstr("200");
                v1[1] = prove_string_from_cstr("OK");
                prove_db_table_add_variant(t, prove_string_from_cstr("Ok"), v1);

                Prove_String *v2[2];
                v2[0] = prove_string_from_cstr("404");
                v2[1] = prove_string_from_cstr("Not Found");
                prove_db_table_add_variant(t, prove_string_from_cstr("NotFound"), v2);

                /* Save */
                Prove_Result sr = prove_db_table_outputs(db, t);
                printf("save_ok=%%d\\n", prove_result_is_ok(sr) ? 1 : 0);
                printf("version=%%lld\\n", (long long)t->version);

                /* Validate */
                printf("exists=%%d\\n",
                    prove_db_table_validates(db, prove_string_from_cstr("status")) ? 1 : 0);

                /* Load */
                Prove_Result lr = prove_db_table_inputs(db, prove_string_from_cstr("status"));
                printf("load_ok=%%d\\n", prove_result_is_ok(lr) ? 1 : 0);
                Prove_DatabaseTable *loaded = (Prove_DatabaseTable *)prove_result_unwrap_ptr(lr);
    ...
    def test_stale_version_rejected(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            ...
            int main(void) {
                ...
                Prove_Database *db = (Prove_Database *)prove_result_unwrap_ptr(
                    prove_db_create(path));
                ...
                prove_db_table_add_variant(t, prove_string_from_cstr("greeting"), v);

                /* Save version 1 */
                prove_db_table_outputs(db, t);
                printf("v1=%%lld\\n", (long long)t->version);
                ...
                /* Load — gets version 2 */
                Prove_DatabaseTable *loaded = (Prove_DatabaseTable *)prove_result_unwrap_ptr(
                    prove_db_table_inputs(db, prove_string_from_cstr("data")));
                printf("loaded_v=%%lld\\n", (long long)loaded->version);

                /* Simulate stale: set loaded version back to 1 */
                loaded->version = 1;
                Prove_Result stale = prove_db_table_outputs(db, loaded);
    ...
    def test_diff_added_removed_changed(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            ...
            int main(void) {
                ...
                /* Old table: A=1, B=2 */
                Prove_DatabaseTable *old = prove_db_table_new(
                    prove_string_from_cstr("t"), 1, cols);
                Prove_String *va[1]; va[0] = prove_string_from_cstr("1");
                prove_db_table_add_variant(old, prove_string_from_cstr("A"), va);
                Prove_String *vb[1]; vb[0] = prove_string_from_cstr("2");
                prove_db_table_add_variant(old, prove_string_from_cstr("B"), vb);

                /* New table: A=10 (changed), C=3 (added), B removed */
                Prove_DatabaseTable *new_t = prove_db_table_new(
                    prove_string_from_cstr("t"), 1, cols);
                Prove_String *va2[1]; va2[0] = prove_string_from_cstr("10");
                prove_db_table_add_variant(new_t, prove_string_from_cstr("A"), va2);
                Prove_String *vc[1]; vc[0] = prove_string_from_cstr("3");
                prove_db_table_add_variant(new_t, prove_string_from_cstr("C"), vc);

                Prove_TableDiff *d = prove_db_diff(old, new_t);
    ...
    def test_patch_apply(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            ...
            int main(void) {
                ...
                /* Base: A=1, B=2 */
                Prove_DatabaseTable *base = prove_db_table_new(
                    prove_string_from_cstr("t"), 1, cols);
                Prove_String *va[1]; va[0] = prove_string_from_cstr("1");
                prove_db_table_add_variant(base, prove_string_from_cstr("A"), va);
                Prove_String *vb[1]; vb[0] = prove_string_from_cstr("2");
                prove_db_table_add_variant(base, prove_string_from_cstr("B"), vb);

                /* Modified: A=10, C=3, B removed */
                Prove_DatabaseTable *modified = prove_db_table_new(
                    prove_string_from_cstr("t"), 1, cols);
                Prove_String *va2[1]; va2[0] = prove_string_from_cstr("10");
                prove_db_table_add_variant(modified, prove_string_from_cstr("A"), va2);
                Prove_String *vc[1]; vc[0] = prove_string_from_cstr("3");
                prove_db_table_add_variant(modified, prove_string_from_cstr("C"), vc);

                Prove_TableDiff *d = prove_db_diff(base, modified);
                Prove_DatabaseTable *patched = prove_db_patch(base, d);
    ...
    def test_integrity_hash(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            ...
            int main(void) {
                ...
                prove_db_table_add_variant(t, prove_string_from_cstr("A"), v);

                Prove_String *h1 = prove_db_integrity(t);
                Prove_String *h2 = prove_db_integrity(t);
                printf("len=%lld\\n", (long long)h1->length);
                printf("eq=%d\\n", prove_string_eq(h1, h2) ? 1 : 0);

                /* Change value, hash should differ */
                v[0] = prove_string_from_cstr("world");
                Prove_DatabaseTable *t2 = prove_db_table_new(
                    prove_string_from_cstr("t"), 1, cols);
                prove_db_table_add_variant(t2, prove_string_from_cstr("A"), v);
                Prove_String *h3 = prove_db_integrity(t2);
    ...
    def test_versions_and_rollback(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            ...
            int main(void) {
                ...
                /* List versions */
                Prove_Result vr = prove_db_version_inputs(db, prove_string_from_cstr("data"));
                ...
                Prove_Result rb = prove_db_rollback(db,
                    prove_string_from_cstr("data"), 1);
                printf("rollback_ok=%%d\\n", prove_result_is_ok(rb) ? 1 : 0);
                Prove_DatabaseTable *rolled = (Prove_DatabaseTable *)prove_result_unwrap_ptr(rb);
    ...
    def test_merge_no_conflict(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            ...
            int main(void) {
                ...
                /* Local changes: A=10 */
                Prove_DatabaseTable *local_t = prove_db_table_new(
                    prove_string_from_cstr("t"), 1, cols);
                Prove_String *vla[1]; vla[0] = prove_string_from_cstr("10");
                prove_db_table_add_variant(local_t, prove_string_from_cstr("A"), vla);
                Prove_String *vlb[1]; vlb[0] = prove_string_from_cstr("2");
                prove_db_table_add_variant(local_t, prove_string_from_cstr("B"), vlb);

                /* Remote changes: B=20 */
                Prove_DatabaseTable *remote_t = prove_db_table_new(
                    prove_string_from_cstr("t"), 1, cols);
                Prove_String *vra[1]; vra[0] = prove_string_from_cstr("1");
                prove_db_table_add_variant(remote_t, prove_string_from_cstr("A"), vra);
                Prove_String *vrb[1]; vrb[0] = prove_string_from_cstr("20");
                prove_db_table_add_variant(remote_t, prove_string_from_cstr("B"), vrb);

                Prove_TableDiff *ld = prove_db_diff(base, local_t);
                Prove_TableDiff *rd = prove_db_diff(base, remote_t);

                Prove_MergeResult *mr = prove_db_merge(base, ld, rd, NULL);
                printf("tag=%d\\n", mr->tag);
                if (mr->tag == PROVE_MERGE_MERGED) {
                    Prove_DatabaseTable *merged = mr->data.table;
...
class TestBinaryCompile:
    def test_binary_output_input(self, tmp_path, runtime_dir):
        code = textwrap.dedent("""\
            #include "prove_database.h"
            #include <stdio.h>
            int main(void) {
                ...
                prove_db_table_add_variant(t, prove_string_from_cstr("Status200"), v1);
                Prove_String *v2[1]; v2[0] = prove_string_from_cstr("Not Found");
                prove_db_table_add_variant(t, prove_string_from_cstr("Status404"), v2);

                prove_db_table_outputs(db, t);

                /* Compile binary */
                Prove_Result br = prove_db_binary_outputs(db,
                ...
                Prove_Result lr = prove_db_binary_inputs(bin_path);
                printf("load_ok=%%d\\n", prove_result_is_ok(lr) ? 1 : 0);

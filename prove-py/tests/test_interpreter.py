...
class TestComptimeInterpreter:
    ...
    def test_unary_negation(self):
        expr = _make_comptime([
            ExprStmt(UnaryExpr("-", IntegerLit("5", S), S), S)
        ])
        result = ComptimeInterpreter().evaluate(expr)
    ...
    def test_unary_not(self):
        expr = _make_comptime([
            ExprStmt(UnaryExpr("not", BooleanLit(True, S), S), S)
        ])
        result = ComptimeInterpreter().evaluate(expr)
    ...
    def test_variable_declaration(self):
        expr = _make_comptime([
            ...
            ExprStmt(
                BinaryExpr(IdentifierExpr("x", S), "+", IntegerLit("5", S), S), S
            ),
        ])
        result = ComptimeInterpreter().evaluate(expr)
    ...
    def test_list_literal(self):
        expr = _make_comptime([
            ExprStmt(
                ListLiteral(
                    [IntegerLit("1", S), IntegerLit("2", S), IntegerLit("3", S)], S
                ),
                S,
            )
        ])
    ...
    def test_string_concat(self):
        expr = _make_comptime([
            ExprStmt(
                BinaryExpr(StringLit("hello", S), "+", StringLit(" world", S), S), S
            )
        ])
        result = ComptimeInterpreter().evaluate(expr)
    ...
    def test_platform_returns_string(self):
        expr = _make_comptime([
            ExprStmt(
                CallExpr(IdentifierExpr("platform", S), [], S), S
            )
        ])
        result = ComptimeInterpreter().evaluate(expr)
        assert isinstance(result.value, str)
        assert result.value in ("linux", "macos", "windows") or isinstance(
            result.value, str
        )
        ...
        expr = _make_comptime([
            ExprStmt(
                MatchExpr(
                    IntegerLit("42", S),
                    [
                        MatchArm(LiteralPattern("1", S, "integer"), [ExprStmt(StringLit("one", S), S)], S),
        ...
        expr = _make_comptime([
            ExprStmt(
                MatchExpr(
                    StringLit("linux", S),
                    [
                        MatchArm(LiteralPattern("linux", S, "string"), [ExprStmt(IntegerLit("4096", S), S)], S),
    ...
    def test_unknown_function(self):
        expr = _make_comptime([
            ExprStmt(
                CallExpr(IdentifierExpr("nonexistent", S), [], S), S
            )
        ])
        with pytest.raises(CompileError):
    ...
    def test_undefined_variable(self):
        expr = _make_comptime([
            ExprStmt(IdentifierExpr("undefined_var", S), S)
        ])
        with pytest.raises(CompileError):
            ComptimeInterpreter().evaluate(expr)

...
| `write` | `transforms write(b Builder, s String) Builder` | Append string |
| `write_char` | `transforms write_char(b Builder, c Character) Builder` | Append char |
...
| Function | Signature | C backing |
|----------|-----------|-----------|
| `new` | `creates new() Table<Value>` | Allocate hash table |
| `has` | `validates has(key String, table Table<Value>) Boolean` | Key exists check |
| `add` | `transforms add(key String, value Value, table Table<Value>) Table<Value>` | Insert/update |
| `get` | `reads get(key String, table Table<Value>) Option<Value>` | Lookup |
| `remove` | `transforms remove(key String, table Table<Value>) Table<Value>` | Delete key |
| `keys` | `reads keys(table Table<Value>) List<String>` | All keys |
| `values` | `reads values(table Table<Value>) List<Value>` | All values |
| `length` | `reads length(table Table<Value>) Integer` | Entry count |

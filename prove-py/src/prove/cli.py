...
def export_treesitter(build: bool) -> None:
    ...
    if generate_treesitter(lists, workspace) and build:
        build_treesitter(workspace)


@export.command(name="pygments")
@click.option("--build", is_flag=True, help="Also run build steps after generating.")
def export_pygments(build: bool) -> None:
    ...
    if generate_pygments(lists, workspace) and build:
        build_pygments(workspace)


@export.command(name="chroma")
@click.option("--build", is_flag=True, help="Also run build steps after generating.")
def export_chroma(build: bool) -> None:
    ...
    if generate_chroma(lists, workspace) and build:
        build_chroma(workspace)


@export.command(name="all")
@click.option("--build", is_flag=True, help="Also run build steps after generating.")
def export_all(build: bool) -> None:
    ...
    click.echo("export: tree-sitter-prove")
    ts_ok = generate_treesitter(lists, workspace)

    click.echo("export: pygments-prove")
    pg_ok = generate_pygments(lists, workspace)

    click.echo("export: chroma-lexer-prove")
    ch_ok = generate_chroma(lists, workspace)

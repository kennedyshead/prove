...
Current state: ~12,400 lines of Python across 25 modules. 506 tests passing.
One backend (C).
...
are implemented (v0.8.3). 506 tests passing.
...
**Python:** `optimizer.py` (new in v0.5, ~400 lines)
...
| Source | 51 | 30 | — |
| Tokens | 188 | 120 | Map |
| Errors | 154 | 100 | Text, InputOutput |
| Ast | 437 | 250 | — |
...
| Lexer export tool | `export.py` | stays in Python | Generates companion project files, no need to self-host |
...
| Risk | Mitigation |
|------|------------|
| Bootstrap compiler bugs | Extensive test suite (506 tests), fix bugs as found during self-hosting |
